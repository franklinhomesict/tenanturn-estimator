...

{
  ...
  const m=run(d),x=m.jobs[0];assert.equal(x.src.workSource,'Blu 2');assert.equal(x.src.pm,'Brandon');
}
{
  const d=baseData(),j=job('blu','2006 S Topeka - Make Ready','Blu');d.jobs=[j];d.docs=[order('o2',j)];
  const m=run(d),x=m.jobs[0];assert.equal(x.src.workSource,'Blu');assert.equal(x.src.pm,'Brandon');
}
{
...
console.log('Source/stage self-test: 5 scenarios passed.');